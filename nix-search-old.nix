...
let
  ...
  searchScript = pkgs.writeScriptBin "search" ''
    ...
    function do_update_cache() {
      ...
      # Query both the attribute name and the description
      ${pkgs.nix}/bin/nix-env -qa '*' --json --description \
        | ${pkgs.jq}/bin/jq -r '
          to_entries[] | 
          select(.value != null) |
          .key + "|" + (.value.description // "No description available") + "|" + (.value.version // "unknown")
        ' > "$CACHE_FILE"
    }
